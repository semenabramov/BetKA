    ...
    const fetchBookmakers = async () => {
        try {
            const response = await apiClient.get(
                API_CONFIG.ENDPOINTS.BOOKMAKERS
            );
            setBookmakers(response.data.data);
        } catch (error) {
            setMessage({
                type: "error",
                text: "Ошибка при загрузке списка букмекеров",
            });
        }
    };
                    ...
                    <TableBody>
                        {bookmakers.map((bookmaker) => (
                            <TableRow key={bookmaker.id}>
                                ...
                                <TableCell>
                                    {renderLeagueLink(bookmaker.league_one_url)}
                                </TableCell>
                                <TableCell>
                                    {renderLeagueLink(bookmaker.league_two_url)}
                                </TableCell>
                                <TableCell>
                                    {renderLeagueLink(
                                ...
                                <TableCell>
                                    {renderLeagueLink(bookmaker.liga_url)}
                                </TableCell>
                                <TableCell>
                                    {renderLeagueLink(bookmaker.la_liga_url)}
                                </TableCell>
                                <TableCell>
                                    {renderLeagueLink(bookmaker.serie_a_url)}
                                </TableCell>
                                <TableCell>
                                    {renderLeagueLink(bookmaker.ligue_one_url)}
                                </TableCell>
                                <TableCell>
                                    <IconButton
                                        color="primary"
                                        onClick={() =>
                                            handleOpenModal(bookmaker)
                                        }
                                    >
                                        <EditIcon />
                                    </IconButton>
